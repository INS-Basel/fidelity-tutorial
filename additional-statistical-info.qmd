...
For a simulation experiment like in 
...
For the first 2 steps the provided R-package {samplingDataCRT} v1.0 [@R-samplingDataCRT] 
is needed, for the last {lme4}. How to install and load see [Introduction]. 
The first steps is done once and the latter two are used repeatedly 
through one simulation experiment within each scenario setting specified in step 1.
...
The resulting reference design matrix can be create manually (by create a corresponding matrix) or by the provided R-function `designMatrix()`.
...
The sampled data can then be analyzed by a **linear mixed model** with the function 
...
## Determine differrent design matrices
...
Within the function the `design` argument is set 'parallel', 
and the parameter `nSW` indicates the number of clusters are being control group. 
...
#no need to show the reader, isn't it?
...
ff<-"FidelityPatterns.png"
ggsave(ff, width = 7, height = 10, dpi=300)
    
```

For our calculation within the simulation we use then these determined fractional values of intervention effects to define the degree of deviation from 100% implementation within the design matrix.
Technically 3 functions in the R-package {fidelitysim} provides the implementation of the different patterns of fidelity. To create a corresponding design matrix using the fidelity patterns also a function is provided for each design.
