...
Figure @fig-workflow provides an overview of the simulation workflow.
...
According to the inner frame of Figure @fig-workflow, several design choices have to be made to evaluate the performance of a cluster randomized trial (Design choice) and with possible implementation error (fidelity pattern). 

Within each simulation experiment steps of data sampling and effect estimation will be repeated. 
For explanation see @sec-statsdetails. To simplify the process of the whole simulation experiment the R-function `simulation()` is provided within the R-package {fidelitysim}. The function includes the steps: data sampling for a specific design and existing implementation error, effect estimation, performance measures calculation. 
...
of the R-package {fidelitysim} calculates and provides the following performance measures of a simulation:  
...
First, make sure that necessary packages are loaded:
...
- Design type controls if individuals will be followed over time (cross-sectional or longitudinal): cross-sectional type, which indicates trials where individuals within the cluster can change over time points
...
If choosing `design = "SWD"` (which is also set by default), the he parameter `nSW` indicates the number of cluster switches per time point from control to intervention group.
...
(designMat_prll <- designMatrix(nC = I, nT = K, nSw = round(I / 2), 
                                design = "parallel"))
...
3 arguments has to be specified to determine the fidelity pattern using the provided function `find.Fidelity.linear`: 
...
(Find the application of the other two functions for slow and fast increase @sec-fidpatterns.)
Create the new design matrix using the function `implemMatrix.parallel` :    
...
This matrix reflect the fidelity pattern, shows the implementation error. Both design matrices are latter used for the simulation as arguments `X = designMat_prll` and `X.A = fidelMat_prll` for the R-function `simulation()` in @sec-simulexp (note, both do not differ in the *perfect* situations). 
...
- Between cluster and error variance $\sigma_c$, $\sigma_e$ which specify the inter-cluster correlation coefficient ICC
-	If applicable: Time trend (effect of each time point during followed time points)

The values has to be choose from expert knowledge on the scientific question, which want to be answered, in relation to the outcome of interest. In general, consideration on sample size calculation is done before conducting a study and determine the parameter.
...
<!-- - a between clusters and error variance regarding the outcome of interest that results in an inter-cluster correlation coefficient (ICC) -->
...
When choosing a longitudinal design instead of a cross-sectional design a third variance need to be specified (see for more detail into @sec-sampdatacrt).
...
(To notice, repeating the simulation `r anzSim` times for this design a computational time of
...
To compare the effect of different fidelity patterns, a repetition of the simulation has to be
conducted.
