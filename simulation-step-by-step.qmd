...
According to the inner frame of @fig-workflow, several specifications have to be made to evaluate the performance of a cluster randomized trial (design choice) with possible implementation error (fidelity pattern). 
...
evaluated for the performance of the model. The provided function `fidelitysim::performanceMeas()` calculates and provides the following performance measures of a simulation: 
...
- Inter-cluster correlation coefficient ICC (between cluster and error variance $\sigma_c$, $\sigma_e$ )  
...
(To notice, repeating the simulation `r anzSim` times for this design a computational time of `r time.sim` min is needed.)
